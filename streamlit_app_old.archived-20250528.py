...
if st.session_state.consent_given and st.session_state.interaction_log:
    ...
    priming_log = run_presence_priming_reflection()

    # Save TXT with YAML block
    with open(filename_txt, "w") as f:
        f.write("# --- Presence Priming Log ---\n\n")
        f.write("primed_on:\n")
        for chapter in priming_log["primed_on"]:
            f.write(f"  - {chapter}\n")
        f.write("\nresonance_log:\n")
        for entry in priming_log["reflection_log"]:
            f.write(f"  - \"{entry}\"\n")
        f.write("\n# ----------------------------\n\n")
        for entry in st.session_state.interaction_log:
    ...
    session_json = {
        "primed_on": priming_log["primed_on"],
        "resonance_log": priming_log["reflection_log"],
